...
export async function POST(request: NextRequest) {
  try {
    ...
    const rows = await prisma.$queryRawUnsafe<Array<{ id: string; enrollmentId: string; expiresAt: Date }>>(`SELECT p."id",p."enrollmentId",p."expiresAt" FROM "ParentAccessToken" p JOIN "Enrollment" e ON e."id"=p."enrollmentId" WHERE p."tokenHash"=$1 AND p."revokedAt" IS NULL AND p."expiresAt">NOW() AND e."status" NOT IN ('WITHDRAWN','TRANSFERRED') LIMIT 1`, hashToken(token));
    if (!rows.length) return NextResponse.json({ error: "This parent access link is invalid, expired, or no longer active." }, { status: 401 });
