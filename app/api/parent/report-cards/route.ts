...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({ schoolName: school?.schoolName || "Aghaaz School", reportCards: releases.map(release => ({ id: release.id, sessionId: release.sessionId, sessionName: release.session.name, releasedAt: release.releasedAt, snapshot: release.snapshot })) });
