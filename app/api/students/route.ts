...
export async function GET(request: NextRequest) {
  try {
    ...
    const visibleStudents = teacherSectionIds ? students.filter(s => s.enrollment?.academicSectionId && teacherSectionIds.includes(s.enrollment.academicSectionId)) : students;
    const ids = visibleStudents.map(s => s.enrollment?.id).filter(Boolean) as string[];
    ...
    return NextResponse.json(visibleStudents.map(s => ({ ...s, registry: s.enrollment ? map.get(s.enrollment.id) : undefined, academic: s.enrollment ? academicMap.get(s.enrollment.id) : undefined })));
  } catch (error) {
    console.error(error);
