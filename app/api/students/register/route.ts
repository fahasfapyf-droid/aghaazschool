...
export async function POST(request:NextRequest) {
  ...
  try {
    ...
    const result=await prisma.$transaction(async tx=>{
      ...
      const enrollment=await tx.enrollment.create({data:{applicationId:application.id,studentId:`STU-${crypto.randomUUID().slice(0,8).toUpperCase()}`,studentIdentityId:studentIdentity.id,admissionNumber:numberSet.admissionNumber,className:selected.gradeName,section:selected.sectionName,academicSessionId:selected.sessionId,academicGradeId:selected.gradeId,academicSectionId:selected.sectionId,status:"ACTIVE"}});
