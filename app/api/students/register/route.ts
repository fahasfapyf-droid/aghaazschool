...
export async function POST(request:NextRequest) {
  ...
  try {
    ...
    const result=await prisma.$transaction(async tx=>{
      const structure=await tx.$queryRawUnsafe<{sessionId:string;sessionName:string;gradeId:string;gradeName:string;gradeCode:string;sectionId:string;sectionName:string;capacity:number|null;enrolledCount:bigint}[]>(`SELECT g."sessionId",s."name" AS "sessionName",g."id" AS "gradeId",g."name" AS "gradeName",g."code" AS "gradeCode",sec."id" AS "sectionId",sec."name" AS "sectionName",sec."capacity",(SELECT COUNT(*) FROM "Enrollment" e WHERE e."section"=sec."name" AND e."className"=g."name" AND e."status"='ENROLLED') AS "enrolledCount" FROM "AcademicGrade" g JOIN "AcademicSession" s ON s."id"=g."sessionId" JOIN "AcademicSection" sec ON sec."gradeId"=g."id" WHERE s."id"=$1 AND g."id"=$2 AND sec."id"=$3 AND g."active"=true AND sec."active"=true LIMIT 1`,sessionId,gradeId,sectionId);
