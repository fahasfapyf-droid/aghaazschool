  ...
  try {
    ...
    const results = await prisma.$transaction(async tx => {
      ...
      for (const entry of body.entries) {
        ...
        const result = await tx.result.upsert({ where: { paperId_studentId: { paperId: body.paperId, studentId: entry.studentId } }, create: { paperId: body.paperId, studentId: entry.studentId, marks, grade: gradingLabelToEnum(resolveGrade(gradingBands, maxMarks ? (marks / maxMarks) * 100 : 0)) as never, gradeLabel: resolveGrade(gradingBands, maxMarks ? (marks / maxMarks) * 100 : 0), remarks: entry.remarks, components: components?.length ? { create: components.map(component => ({ name: component.name, maxMarks: component.maxMarks, marks: component.marks })) } : undefined }, update: { marks, grade: gradingLabelToEnum(resolveGrade(gradingBands, maxMarks ? (marks / maxMarks) * 100 : 0)) as never, gradeLabel: resolveGrade(gradingBands, maxMarks ? (marks / maxMarks) * 100 : 0), remarks: entry.remarks, components: components ? { deleteMany: {}, create: components.map(component => ({ name: component.name, maxMarks: component.maxMarks, marks: component.marks })) } : undefined }, select: { id: true, studentId: true, marks: true, grade: true, gradeLabel: true } });
        saved.push(result);
      }
      return saved;
    });
