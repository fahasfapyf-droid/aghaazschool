...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await prisma.result.upsert({ where: { paperId_studentId: { paperId: body.paperId, studentId: body.studentId } }, create: { paperId: body.paperId, studentId: body.studentId, marks, grade: resolvedGrade as never, gradeLabel: resolvedLabel, remarks: body.remarks, components: components?.length ? { create: components.map(c => ({ name: c.name, maxMarks: c.maxMarks, marks: c.marks })) } : undefined }, update: { marks, grade: resolvedGrade as never, gradeLabel: resolvedLabel, remarks: body.remarks, components: components ? { deleteMany: {}, create: components.map(c => ({ name: c.name, maxMarks: c.maxMarks, marks: c.marks })) } : undefined }, include: { components: true, paper: true } });
