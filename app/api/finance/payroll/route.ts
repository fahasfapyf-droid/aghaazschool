...
const schema = z.object({ payrollMonth: z.string().min(1), staffName: z.string().trim().min(1).max(200), grossAmount: z.coerce.number().positive(), adjustment: z.coerce.number().min(0).default(0), netAmount: z.coerce.number().positive(), paymentStatus: z.nativeEnum(FinanceStatus), paymentDate: z.string().optional(), paymentMethod: z.nativeEnum(FinancePaymentMethod).optional(), notes: z.string().trim().max(1000).optional() });

export async function GET(request: NextRequest) {
  const user = await getCurrentUser();
  if (!user) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!roleAllowed(user.role, [...roles])) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  const rows = await prisma.payrollRecord.findMany({ orderBy: [{ payrollMonth: "desc" }, { staffName: "asc" }], take: 500 });
...
export async function POST(request: NextRequest) {
  ...
  try {
    const row = await prisma.payrollRecord.upsert({ where: { payrollMonth_staffName: { payrollMonth: month, staffName: parsed.data.staffName } }, update: { grossAmount: parsed.data.grossAmount, adjustment: parsed.data.adjustment, netAmount: parsed.data.netAmount, paymentStatus: parsed.data.paymentStatus, paymentDate, paymentMethod: parsed.data.paymentMethod || null, notes: parsed.data.notes || null }, create: { payrollMonth: month, staffName: parsed.data.staffName, grossAmount: parsed.data.grossAmount, adjustment: parsed.data.adjustment, netAmount: parsed.data.netAmount, paymentStatus: parsed.data.paymentStatus, paymentDate, paymentMethod: parsed.data.paymentMethod || null, notes: parsed.data.notes || null, createdBy: user.id } });
    await writeAuditLog({ userId: user.id, action: "PAYROLL_RECORD_SAVED", entityType: "PayrollRecord", entityId: row.id, metadata: { staffName: row.staffName, payrollMonth: row.payrollMonth.toISOString(), netAmount: Number(row.netAmount), paymentStatus: row.paymentStatus }, context: requestAuditContext(request) });
