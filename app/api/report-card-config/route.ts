...
export async function POST(request: NextRequest) {
  ...
  const normalizedComponents: NormalizedComponent[] = components.map((component: unknown, index: number) => {
    ...
    return { name: typeof item.name === "string" ? item.name.trim() : "", maxMarks: Number(item.maxMarks), displayOrder: Number.isInteger(item.displayOrder) ? Number(item.displayOrder) : index };
  });
  if (normalizedComponents.some(c => !c.name || !Number.isFinite(c.maxMarks) || c.maxMarks <= 0)) return NextResponse.json({ error: "Every assessment component needs a name and positive max marks" }, { status: 400 });
  const componentTotal = normalizedComponents.reduce((sum, c) => sum + c.maxMarks, 0);
  if (normalizedComponents.length && Math.abs(componentTotal - maxMarks) > 0.001) return NextResponse.json({ error: "Assessment component maximums must equal the subject maximum marks" }, { status: 400 });
