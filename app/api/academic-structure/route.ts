...
const inputSchema = z.discriminatedUnion("entity", [
 z.object({entity:z.literal("session"),name:z.string().min(4).max(50),startDate:z.string().date(),endDate:z.string().date()}),
...
export async function POST(request:NextRequest){
 try{
  ...
  if(input.entity==="session"){
   await prisma.academicSession.create({data:{id,name:input.name,startDate:new Date(input.startDate),endDate:new Date(input.endDate)}});
  }else if(input.entity==="term"){
   await prisma.$executeRawUnsafe(`INSERT INTO "AcademicTermRecord" ("id","sessionId","name","termKey","startDate","endDate","updatedAt") VALUES ($1,$2,$3,$4,$5,$6,CURRENT_TIMESTAMP)`,id,input.sessionId,input.name,input.termKey,input.startDate,input.endDate);
  }else if(input.entity==="grade"){
   await prisma.$executeRawUnsafe(`INSERT INTO "AcademicGrade" ("id","sessionId","name","code","displayOrder","updatedAt") VALUES ($1,$2,$3,$4,$5,CURRENT_TIMESTAMP)`,id,input.sessionId,input.name,input.code,input.displayOrder);
  }else if(input.entity==="section"){
   if(input.classTeacherStaffId){
    const staff=await prisma.staff.findUnique({where:{id:input.classTeacherStaffId},select:{id:true,active:true,staffType:true}});
    if(!staff?.active||staff.staffType!=="TEACHER")return NextResponse.json({error:"Select an active teacher."},{status:400});
   }
   await prisma.$executeRawUnsafe(`INSERT INTO "AcademicSection" ("id","gradeId","name","capacity","classTeacherStaffId","updatedAt") VALUES ($1,$2,$3,$4,$5,CURRENT_TIMESTAMP)`,id,input.gradeId,input.name,input.capacity??null,input.classTeacherStaffId??null);
   if(input.classTeacherStaffId)await prisma.$executeRawUnsafe(`INSERT INTO "AcademicClassTeacher" ("id","sectionId","staffId","startDate","updatedAt") VALUES ($1,$2,$3,CURRENT_DATE,CURRENT_TIMESTAMP)`,randomUUID(),id,input.classTeacherStaffId);
  }else if(input.entity==="subject"){
   await prisma.$executeRawUnsafe(`INSERT INTO "AcademicSubject" ("id","code","name","shortName","updatedAt") VALUES ($1,$2,$3,$4,CURRENT_TIMESTAMP)`,id,input.code,input.name,input.shortName??null);
  }else if(input.entity==="classSubject"){
   if(input.teacherStaffId){
    const staff=await prisma.staff.findUnique({where:{id:input.teacherStaffId},select:{id:true,active:true,staffType:true}});
    if(!staff?.active||staff.staffType!=="TEACHER")return NextResponse.json({error:"Select an active teacher."},{status:400});
   }
   const existing=await prisma.$queryRawUnsafe<Array<{id:string}>>(`SELECT "id" FROM "AcademicClassSubject" WHERE "gradeId"=$1 AND "subjectId"=$2 AND ("termId"=$3 OR ("termId" IS NULL AND $3 IS NULL)) LIMIT 1`,input.gradeId,input.subjectId,input.termId??null);
   if(existing[0]){
    await prisma.$executeRawUnsafe(`UPDATE "AcademicClassSubject" SET "teacherStaffId"=$1,"maxMarks"=$2,"active"=true,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$3`,input.teacherStaffId??null,input.maxMarks??null,existing[0].id);
   }else{
    await prisma.$executeRawUnsafe(`INSERT INTO "AcademicClassSubject" ("id","gradeId","subjectId","termId","teacherStaffId","maxMarks","displayOrder","updatedAt") VALUES ($1,$2,$3,$4,$5,$6,$7,CURRENT_TIMESTAMP)`,id,input.gradeId,input.subjectId,input.termId??null,input.teacherStaffId??null,input.maxMarks??null,input.displayOrder);
   }
  }else{
   const staff=await prisma.staff.findUnique({where:{id:input.staffId},select:{id:true,active:true,staffType:true}});
   if(!staff?.active||staff.staffType!=="TEACHER")return NextResponse.json({error:"Select an active teacher."},{status:400});
   const section=await prisma.$queryRawUnsafe<Array<{id:string;gradeId:string}>>(`SELECT "id","gradeId" FROM "AcademicSection" WHERE "id"=$1 AND "active"=true LIMIT 1`,input.sectionId);
   if(!section[0])return NextResponse.json({error:"Section not found or inactive."},{status:404});
   await prisma.$transaction(async tx=>{
    await tx.$executeRawUnsafe(`UPDATE "AcademicClassTeacher" SET "active"=false,"endDate"=COALESCE($2::date,CURRENT_DATE),"updatedAt"=CURRENT_TIMESTAMP WHERE "sectionId"=$1 AND "active"=true`,input.sectionId,input.startDate??null);
    await tx.$executeRawUnsafe(`UPDATE "AcademicSection" SET "classTeacherStaffId"=$1,"updatedAt"=CURRENT_TIMESTAMP WHERE "id"=$2`,input.staffId,input.sectionId);
    await tx.$executeRawUnsafe(`INSERT INTO "AcademicClassTeacher" ("id","sectionId","staffId","startDate","endDate","updatedAt") VALUES ($1,$2,$3,COALESCE($4::date,CURRENT_DATE),$5,CURRENT_TIMESTAMP)`,id,input.sectionId,input.staffId,input.startDate??null,input.endDate??null);
   });
  }
