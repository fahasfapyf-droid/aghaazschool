...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const user = await prisma.$transaction(async tx => {
      const created = await tx.user.create({ data: { name, phone, email, role, passwordHash: createPasswordHash(password) }, select: { id: true, name: true, email: true, role: true, active: true, updatedAt: true } });
