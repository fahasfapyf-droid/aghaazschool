...
export default function ExamDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const load = async () => {
    ...
    x.papers.forEach((p: Paper) => p.results.forEach(r => {
      nextMarks[`${p.id}:${r.studentId}`] = r.marks;
      if (r.components?.length) nextComponents[`${p.id}:${r.studentId}`] = Object.fromEntries(r.components.map(c => [c.name, c.marks]));
    }));
    setMarks(nextMarks);
  ...
  async function save(p: Paper, s: Student) {
    ...
    const payload = components ? { paperId: p.id, studentId, components } : { paperId: p.id, studentId, marks: marks[key] };
  ...
  return <main className="admissions-shell">
    ...
    {exam.papers.map(p => {
      ...
      return <section className="applications-card" key={p.id}>
        ...
        <div className="table-wrap"><table><thead><tr><th>Student</th>{config?.components?.length ? config.components.map(c => <th key={c.name}>{c.name}<small>/ {c.maxMarks}</small></th>) : <th>Marks</th>}<th>Grade</th><th></th></tr></thead>
          <tbody>{students.filter(s => !s.enrollment?.className || s.enrollment.className === p.className).map(s => {
            ...
            return <tr key={s.id}><td><strong>{s.application?.studentName || "—"}</strong></td>
              ...
              <td>{result?.grade || "—"}</td><td><button className="row-action" disabled={saving === key} onClick={() => save(p, s)}>{saving === key ? "Saving…" : "Save"}</button></td></tr>;
          })}</tbody>
        </table></div>
