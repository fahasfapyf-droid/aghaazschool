...
type SubjectReport = { subject: string; maxMarks: number; marks: number; percentage: number; grade: string | null; components: ComponentReport[] };
...
export default function ReportCards() {
  ...
  useEffect(() => {
    fetch("/api/students").then(r => r.ok ? r.json() : Promise.reject()).then(setStudents).catch(() => setError("Unable to load students"));
  }, []);
  ...
  return <main className="container report-page">
    <header className="admissions-header report-toolbar">
      ...
      <div className="report-controls">
        {report && <button className="button secondary" type="button" onClick={() => window.print()}>Print Report Card</button>}
    ...
    {report && <article className="official-report" id="report-card">
      ...
      <section className="remarks"><h3>Teacher's Remarks</h3><div></div></section>
