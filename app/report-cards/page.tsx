...
type SubjectReport = { subject: string; maxMarks: number; marks: number; percentage: number; grade: string | null; remarks?: string | null; components: ComponentReport[] };
type TermReport = { name: string; subjects: SubjectReport[]; totalMarks: number; obtainedMarks: number };
type Report = {
  ...
  terms: TermReport[];
  final: { totalMarks: number; obtainedMarks: number; percentage: number; grade: string | null; position: number | null };
  attendance: { total: number; present: number; absent: number; leave: number; percentage: number };
};

const gradeLabel = (grade: string | null) => ({ A_PLUS: "A+", A: "A", B_PLUS: "B+", B: "B", C: "C", D: "D", TRY_AGAIN: "TRY AGAIN" }[grade || ""] || "—");
const gradeFromPercentage = (p: number) => p <= 0 ? null : p >= 90 ? "A_PLUS" : p >= 80 ? "A" : p >= 70 ? "B_PLUS" : p >= 60 ? "B" : p >= 50 ? "C" : p >= 40 ? "D" : "TRY_AGAIN";
...
export default function ReportCards() {
  ...
  useEffect(() => {
    fetch("/api/students").then(r => r.ok ? r.json() : Promise.reject()).then(list => {
      setStudents(list);
  ...
  async function load(id: string) {
    setSelected(id); setReport(null); setError(""); if (!id) return;
    setLoading(true);
    try {
      ...
      setReport(data);
    } catch (e) { setError(e instanceof Error ? e.message : "Unable to load report card"); }
    finally { setLoading(false); }
  }

  const annualGrade = useMemo(() => report ? gradeLabel(report.final.grade || gradeFromPercentage(report.final.percentage)) : "—", [report]);
  const remarks = useMemo(() => report?.terms.flatMap(term => term.subjects.filter(subject => subject.remarks).map(subject => ({ term: term.name, subject: subject.subject, remark: subject.remarks as string }))) || [], [report]);

  return <main className="container report-page">
    <header className="admissions-header report-toolbar">
      <div><div className="eyebrow">Aghaaz School Management / Academic Reports</div><h1>Report Cards</h1><p>Official three-term student report card and annual result.</p></div>
      <div className="report-controls">
        {report && <button className="button secondary" type="button" onClick={() => window.print()}>Print / Save PDF</button>}
        <select className="filter-select" value={selected} onChange={e => load(e.target.value)}><option value="">Select student</option>{students.map(s => <option key={s.id} value={s.id}>{s.application?.studentName || "Unnamed"} — {s.admissionNumber || s.id}</option>)}</select>
      </div>
    </header>
    {error && <div className="error">{error}</div>}
    {loading && <div className="empty-state">Preparing official report card…</div>}

    {report && <article className="official-report" id="report-card">
      <div className="report-school-head">
        <div className="school-emblem">A</div>
        <div><div className="report-school-name">AGHAAZ SCHOOL</div><div className="report-school-subtitle">STUDENT ACADEMIC REPORT CARD</div><div className="report-session">Academic Session {report.student.session}</div></div>
      </div>
