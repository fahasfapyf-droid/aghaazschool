...
type Workspace = { staff: Teacher & { designation: string }; teachers: Teacher[]; date: string; today: string; schedule: Schedule[]; todaySchedule: Schedule[]; classes: ClassSummary[]; homework: Homework[]; actions: Action[] };
...
export default function TeacherWorkspacePage() {
  ...
  return <main className="admissions-shell">
    ...
    {loading ? <section className="empty-state">Loading teacher workspace…</section> : data && <>
      ...
      <section className="applications-card"><div className="table-toolbar"><div><h2>My action queue</h2><p>Monitor items assigned to this teacher.</p></div><Link className="row-action" href="/monitor">Open Monitor</Link></div>{data.actions.length === 0 ? <div className="empty-state">No open Monitor actions assigned.</div> : <div className="table-wrap"><table><thead><tr><th>Action</th><th>Status</th><th>Due</th><th></th></tr></thead><tbody>{data.actions.map(x => <tr key={x.id}><td><strong>{x.title}</strong><small>{x.description}</small></td><td>{x.status}</td><td>{x.dueDate ? new Date(x.dueDate).toLocaleDateString() : "No due date"}</td><td><Link className="row-action" href="/monitor">Open queue</Link></td></tr>)}</tbody></table></div>}</section>
    </>}
  </main>;
}
