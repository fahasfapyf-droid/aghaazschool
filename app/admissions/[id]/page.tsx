...
export default function AdmissionDetail(){
 ...
 function submitAssessment(e:FormEvent){
   submitAction(e,{action:"assessment",type:assessment.type,scheduledAt:assessment.scheduledAt?new Date(assessment.scheduledAt).toISOString():"",evaluator:assessment.evaluator});
 }
 function submitEnrollment(e:FormEvent){
   submitAction(e,{action:"enrollment",studentId:enrollment.studentId,admissionNumber:enrollment.admissionNumber,className:enrollment.className||app?.desiredClass||"",section:enrollment.section});
 }
 ...
 return <main className="container admission-detail">
  ...
  <section className="card workflow-card"><div className="panel-heading"><div><h2>Admission workflow</h2><p>Complete the operational steps without leaving the application.</p></div></div><div className="workflow-grid">
   <form className="workflow-form" onSubmit={e=>submitAction(e,{action:"document",...doc})}><h3>Add document</h3><input className="input" required placeholder="Document type" value={doc.documentType} onChange={e=>setDoc({...doc,documentType:e.target.value})}/><input className="input" required placeholder="File reference / URL" value={doc.fileReference} onChange={e=>setDoc({...doc,fileReference:e.target.value})}/><button className="button" disabled={action==="document"}>{action==="document"?"Adding…":"Add document"}</button></form>
   <form className="workflow-form" onSubmit={submitAssessment}><h3>Schedule assessment</h3><select className="input" value={assessment.type} onChange={e=>setAssessment({...assessment,type:e.target.value})}><option value="TEST">Test</option><option value="INTERVIEW">Interview</option></select><input className="input" required type="datetime-local" value={assessment.scheduledAt} onChange={e=>setAssessment({...assessment,scheduledAt:e.target.value})}/><input className="input" placeholder="Evaluator" value={assessment.evaluator} onChange={e=>setAssessment({...assessment,evaluator:e.target.value})}/><button className="button" disabled={action==="assessment"}>{action==="assessment"?"Scheduling…":"Schedule"}</button></form>
   <form className="workflow-form" onSubmit={e=>submitAction(e,{action:"payment",...payment})}><h3>Record payment</h3><input className="input" required placeholder="Fee type" value={payment.feeType} onChange={e=>setPayment({...payment,feeType:e.target.value})}/><div className="form-row"><input className="input" required type="number" min="0" step="0.01" placeholder="Amount" value={payment.amount} onChange={e=>setPayment({...payment,amount:e.target.value})}/><input className="input" type="number" min="0" step="0.01" placeholder="Discount" value={payment.discount} onChange={e=>setPayment({...payment,discount:e.target.value})}/></div><input className="input" placeholder="Receipt number" value={payment.receiptNumber} onChange={e=>setPayment({...payment,receiptNumber:e.target.value})}/><button className="button" disabled={action==="payment"}>{action==="payment"?"Recording…":"Record payment"}</button></form>
   <form className="workflow-form" onSubmit={e=>submitAction(e,{action:"decision",...decision})}><h3>Record decision</h3><select className="input" value={decision.decision} onChange={e=>setDecision({...decision,decision:e.target.value})}><option value="APPROVED">Approve</option><option value="WAITLISTED">Waitlist</option><option value="REJECTED">Reject</option></select><input className="input" required placeholder="Decided by" value={decision.decidedBy} onChange={e=>setDecision({...decision,decidedBy:e.target.value})}/><button className="button" disabled={action==="decision"}>{action==="decision"?"Saving…":"Save decision"}</button></form>
   <form className="workflow-form" onSubmit={submitEnrollment}><h3>Enroll student</h3><input className="input" required placeholder="Student ID" value={enrollment.studentId} onChange={e=>setEnrollment({...enrollment,studentId:e.target.value})}/><input className="input" required placeholder="Admission number" value={enrollment.admissionNumber} onChange={e=>setEnrollment({...enrollment,admissionNumber:e.target.value})}/><div className="form-row"><input className="input" required placeholder="Class" value={enrollment.className||app.desiredClass||""} onChange={e=>setEnrollment({...enrollment,className:e.target.value})}/><input className="input" placeholder="Section" value={enrollment.section} onChange={e=>setEnrollment({...enrollment,section:e.target.value})}/></div><button className="button" disabled={action==="enrollment"}>{action==="enrollment"?"Enrolling…":"Enroll student"}</button></form>
  </div></section>
