...
export default function AdmissionDetail(){
 ...
 return <main className="container admission-detail">
  ...
  <header className="detail-header"><div><div className="eyebrow">Application {app.applicationNumber||app.id}</div><h1>{app.studentName||"Applicant"}</h1><p className="muted">{app.desiredClass||"Class not specified"} · {app.session?.name||"2026–27"} · Received {app.createdAt?new Date(app.createdAt).toLocaleDateString():"—"}</p></div><div className="detail-actions"><button className="button" disabled={saving||editSaving} onClick={()=>{setEditForm(buildEditForm(app));setEditing(true);setError("");setSuccess("");}}>Edit application</button><select className="filter-select" value={app.status} disabled={saving} onChange={e=>changeStatus(e.target.value)}>{statuses.map(s=><option key={s} value={s}>{label(s)}</option>)}</select><button className="button secondary" disabled={saving||loading} onClick={load}>{loading?"Loading…":"Refresh"}</button></div></header>
