...
export default function BulkResultEntry() {
  ...
  useEffect(() => {
    ...
    }).catch(e => setError(e instanceof Error ? e.message : "Unable to load academic setup")).finally(() => setLoading(false));
  }, []);
  ...
  const sections = useMemo(() => {
    const values = students.filter(student => !paper || student.className === paper.className).map(student => student.section).filter((value): value is string => Boolean(value));
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const configParams = new URLSearchParams({ sessionId: exam.sessionId, className: paper.className, term: exam.term || "" });
        ...
        const subject = (Array.isArray(configData.subjects) ? configData.subjects : []).find((item: SubjectConfig) => item.subject.toLowerCase() === paper.subject.toLowerCase()) || null;
        setConfig(subject);
  ...
  useEffect(() => {
    if (!paper) return;
    const resultByStudent = new Map(existing.filter(result => result.paper.id === paper.id).map(result => [result.studentId, result]));
    const next: Record<string, Entry> = {};
    students.filter(student => student.className === paper.className && (!section || student.section === section)).forEach(student => {
  ...
  const setComponent = (studentId: string, name: string, value: string) => {
    setEntries(current => ({
      ...current,
      [studentId]: { ...current[studentId], components: { ...current[studentId]?.components, [name]: value } },
    }));
  };

  const totalFor = (entry: Entry) => config?.components.length
    ? config.components.reduce((sum, component) => sum + Number(entry.components[component.name] || 0), 0)
    : Number(entry.marks || 0);

  const visibleEntries = classStudents.map(student => entries[student.id]).filter(Boolean);
  const enteredCount = visibleEntries.filter(entry => totalFor(entry) > 0).length;
  const average = visibleEntries.length ? visibleEntries.reduce((sum, entry) => sum + totalFor(entry), 0) / visibleEntries.length : 0;
  ...
  const save = async () => {
    if (!paper || !visibleEntries.length) return setError("Select an examination, subject and class students first.");
    setSaving(true);
    setMessage("");
    setError("");
    try {
      const payload = visibleEntries.map(entry => ({
        ...
        marks: config?.components.length ? undefined : Number(entry.marks || 0),
        components: config?.components.length ? config.components.map(component => ({ name: component.name, maxMarks: Number(component.maxMarks), marks: Number(entry.components[component.name] || 0) })) : undefined,
      ...
      setMessage(`${data.saved} student results saved successfully.`);
      const refreshed = await fetch(`/api/results?examId=${encodeURIComponent(exam?.id || "")}`).then(response => response.json()).catch(() => []);
  ...
  return <main className="container bulk-results">
    <header className="admissions-header">
      <div><div className="eyebrow">Aghaaz School Management / Results</div><h1>Bulk Class Result Entry</h1><p>Enter a complete subject result sheet for the selected class or section.</p></div>
      <div className="actions"><Link className="button secondary" href="/results">Results</Link><Link className="button secondary" href="/results/entry">Single Entry</Link><Link className="button secondary" href="/report-cards">Report Cards</Link></div>
    </header>

    {loading ? <div className="empty-state">Loading examination setup…</div> : <>
      ...
      {paper && <section className="applications-card sheet">
        <div className="sheet-head"><div><div className="eyebrow">{paper.className}{section ? ` · Section ${section}` : ""}</div><h2>{paper.subject}</h2><p>{exam?.name} · Maximum {paper.maxMarks} marks</p></div><div className="metrics"><span><b>{enteredCount}</b> / {visibleEntries.length} entered</span><span>Average <b>{average.toFixed(2)}</b></span></div></div>
        <div className="table-wrap"><table><thead><tr><th>#</th><th>Student</th>{config?.components.length ? config.components.map(component => <th key={component.name}>{component.name}<small>/{component.maxMarks}</small></th>) : <th>Obtained / {paper.maxMarks}</th>}<th>Grade</th><th>Remarks</th></tr></thead><tbody>
          {classStudents.map((student, index) => { const entry = entries[student.id]; const total = entry ? totalFor(entry) : 0; return <tr key={student.id}><td>{index + 1}</td><td className="student">{student.name}<small>{student.section || ""}</small></td>{config?.components.length ? config.components.map(component => <td key={component.name}><input className="cell-input" type="number" min="0" max={component.maxMarks} step="0.01" value={entry?.components[component.name] || ""} onChange={e => setComponent(student.id, component.name, e.target.value)} /></td>) : <td><input className="cell-input" type="number" min="0" max={paper.maxMarks} step="0.01" value={entry?.marks || ""} onChange={e => setValue(student.id, { marks: e.target.value })} /></td>}<td className="grade">{grade(total, Number(paper.maxMarks))}</td><td><input className="cell-input remark" value={entry?.remarks || ""} maxLength={2000} onChange={e => setValue(student.id, { remarks: e.target.value })} placeholder="Optional" /></td></tr> })}
